...
public sealed class Step16ClassWeaponFoundationTests
{
    ...
    [Test] public void SchemaSevenMigrationAddsOnlyStableClassAndSwordWeaponIdentity()
    {
        ...
        try
        {
            ...
            Assert.That(migrated.schemaVersion,Is.EqualTo(10));Assert.That(migrated.payload.availablePassivePoints,Is.EqualTo(1));Assert.That(migrated.payload.baseClassId,Is.EqualTo(PlayerClassIds.Warrior));Assert.That(migrated.payload.gearItems[0].weaponTypeId,Is.EqualTo(WeaponTypeIds.Sword));Assert.That(migrated.payload.gearItems[0].baseDamageMin,Is.EqualTo(18));
        }
