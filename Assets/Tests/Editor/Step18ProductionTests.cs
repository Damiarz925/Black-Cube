...
public sealed class Step18ProductionTests
{
    ...
    {Assert.That(PassiveTreeDefinition.NodeCount,Is.EqualTo(366));Assert.That(PassiveTreeDefinition.Nodes.SelectMany(x=>x.Effects).Any(x=>x.Stat==StatTypes.CooldownReduction),Is.True);Assert.That(PassiveTreeDefinition.Nodes.SelectMany(x=>x.Effects).Any(x=>x.Stat==StatTypes.ShockEffect),Is.True);Assert.That(PassiveTreeDefinition.Nodes.SelectMany(x=>x.Effects).Any(x=>x.Stat==StatTypes.ChillEffect),Is.True);Assert.That(PassiveTreeDefinition.Nodes.SelectMany(x=>x.Effects).Any(x=>x.Stat==StatTypes.AuraEffect),Is.True);}

    [Test] public void SchemaNineMigratesToTenWithEmptyPermanentConfiguration()
    {
        string path=Path.Combine(Path.GetTempPath(),"BlackCube-Step18-"+Guid.NewGuid().ToString("N")+".json");try{var payload=new GameStatePayload{playerLevel=1,availablePassivePoints=1,encounterStartLife=100,encounterStartMana=100};for(int i=0;i<RelicInventory.ActiveSlotCount;i++)payload.activeRelicIds.Add(string.Empty);var envelope=new SaveEnvelope{schemaVersion=9,runId="step18",runSeed=18,savedAtUtc=DateTime.UtcNow.ToString("O"),payload=payload};File.WriteAllText(path,JsonUtility.ToJson(envelope));Assert.That(GamePersistence.TryReadFile(path,out var migrated,out var error),Is.True,error);Assert.That(migrated.schemaVersion,Is.EqualTo(GamePersistence.SchemaVersion));Assert.That(migrated.payload.transformedPassiveNodeIds,Is.Empty);Assert.That(migrated.payload.subclassProjectileMode,Is.EqualTo(SubclassProjectileMode.Volley));}finally{if(File.Exists(path))File.Delete(path);}
    }

    [Test] public void TransformationsReplaceOnlyAllocatedConnectedNonStructuralNodes()
    {
        var go=new GameObject("transformations");try{var identity=go.AddComponent<PlayerIdentityState>();identity.BeginNewGame(PlayerClassIds.Warrior);identity.CompleteMilestone(PlayerIdentityState.StoryCompletionMilestoneId);Assert.That(identity.SelectSubclass(SubclassIds.WarriorBleed),Is.True);var progression=go.AddComponent<PlayerProgression>();int first=PassiveTreeDefinition.AdjacentNodeIds(PassiveTreeDefinition.StartNodeId(PlayerClassIds.Warrior))[0];int second=PassiveTreeDefinition.AdjacentNodeIds(first).First(x=>!PassiveTreeDefinition.IsClassStart(x));var ranks=new int[PassiveTreeDefinition.NodeCount];ranks[first]=ranks[second]=1;Assert.That(progression.RestoreProgression(3,0,1,ranks),Is.True);Assert.That(progression.TryTransform(first),Is.True);Assert.That(progression.TryTransform(second),Is.True);Assert.That(progression.TransformedCount,Is.EqualTo(2));Assert.That(progression.TryRemoveTransformation(first),Is.True);Assert.That(progression.TransformedCount,Is.EqualTo(1));progression.ClearTransformations();Assert.That(progression.TransformedCount,Is.Zero);}finally{UnityEngine.Object.DestroyImmediate(go);}
    }
}
