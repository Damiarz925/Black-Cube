...
public sealed class Step17FoundationTests
{
    ...
    [Test] public void SchemaNineMigrationClearsV1TreeAndRefundsLevelTotal()
    {
        string path=Path.Combine(Path.GetTempPath(),"BlackCubeStep17Migration-"+Guid.NewGuid().ToString("N")+".json");try{var e=new SaveEnvelope{schemaVersion=8,runId="run",runSeed=2,savedAtUtc=DateTime.UtcNow.ToString("O"),payload=new GameStatePayload{playerLevel=72,availablePassivePoints=0,encounterStartLife=100,encounterStartMana=100}};e.payload.passiveRanks.Add(new PassiveRankData(0,1));for(int i=0;i<RelicInventory.ActiveSlotCount;i++)e.payload.activeRelicIds.Add(string.Empty);File.WriteAllText(path,JsonUtility.ToJson(e));Assert.That(GamePersistence.TryReadFile(path,out var migrated,out var error),Is.True,error);Assert.That(migrated.schemaVersion,Is.EqualTo(9));Assert.That(migrated.payload.passiveRanks,Is.Empty);Assert.That(migrated.payload.availablePassivePoints,Is.EqualTo(72));}finally{if(File.Exists(path))File.Delete(path);}
    }
