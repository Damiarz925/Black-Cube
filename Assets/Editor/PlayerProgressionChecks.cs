...
public static class PlayerProgressionChecks
{
    ...
    static IEnumerator Run()
    {
        ...
        yield return null;
        Check(hp.MaxLife==1000 && player.EquippedWeaponBaseDamage>0,$"Actual baseline health and generated starter weapon ({hp.MaxLife:0}/{player.EquippedWeaponBaseDamage:0.##})");
        hp.LoseLife(500);
        double partial=xp.RequiredXp*.5;xp.AddExperience(partial);
        Check(xp.Experience==partial && xp.Level==1 && hp.CurrentLife==500,"Partial XP does not heal");
        xp.AddExperience(xp.RequiredXp-xp.Experience);
        Check(xp.Level==2 && xp.AvailablePoints==1 && xp.Experience==0 && hp.CurrentLife==1000,"Completing the level grants one point and heals");
        ...
        Check(lootDelta>0 && lootDelta<kills,"Natural equipment rewards follow the probabilistic drop table");
        ...
        Write($"PASS cleanup: one enemy after delayed destruction, {lootDelta} probabilistic equipment rewards for {kills} kills, no new errors.");
        var ui=Object.FindFirstObjectByType<SkillTreeUI>();ui.Toggle();
        yield return null;
