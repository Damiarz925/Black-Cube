...
public static class StatusModelChecks
{
    ...
    static void Verify()
    {
        ...
        var player=Object.FindFirstObjectByType<PlayerController>();var liveController=player.GetComponent<StatusController>();var liveHp=player.GetComponent<HealthComponent>();
        var model=new GameObject("Deterministic status model");model.tag="Player";
        model.AddComponent<StatsComponent>();model.AddComponent<PlayerStatSetup>();model.AddComponent<HealthComponent>();model.AddComponent<DamageReceiver>();model.AddComponent<PlayerController>();model.AddComponent<StatusController>();
        typeof(StatusController).GetMethod("Start",BindingFlags.Instance|BindingFlags.NonPublic).Invoke(model.GetComponent<StatusController>(),null);
        var controller=model.GetComponent<StatusController>();var hp=model.GetComponent<HealthComponent>();var stats=model.GetComponent<StatsComponent>();
        ...
        liveHp.ReviveToFullLife();liveController.ApplyStatus(poison,1,10,5,stats,1);liveHp.LoseLife(2000);Require(liveController.GetStatusSummaries().Count==0,"Death clears player statuses");
        ...
        liveController.ApplyStatus(poison,1,100,3,stats,4);liveController.ApplyStatus(poison,1,50,4,stats,4);liveController.ApplyStatus(poison,1,75,5,stats,4);
        ...
        {var effect=AssetDatabase.LoadAssetAtPath<StatusEffects>("Assets/Prefabs/Scriptable Objects/"+name+".asset");liveController.ApplyStatus(effect,1,10,4,stats,1);newEnemy.ApplyStatus(effect,1,8,4,stats,1);}
        var chill=ScriptableObject.CreateInstance<StatusEffects>();var so=new SerializedObject(chill);so.FindProperty("statusType").enumValueIndex=2;so.FindProperty("stackPolicy").enumValueIndex=3;so.ApplyModifiedPropertiesWithoutUndo();liveController.ApplyStatus(chill,1,.2f,3,stats,1);Object.Destroy(model);
        Time.timeScale=0;
