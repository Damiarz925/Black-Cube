...
public static class PassiveTreeDefinition
{
    ...
    public static string DisplayName(PassiveBranch b)=>b switch{PassiveBranch.CastSpeed=>"Cooldown Reduction",PassiveBranch.Poison=>"Void Damage",PassiveBranch.IncreasedProjectileAmount=>"Additional Projectiles",PassiveBranch.ChanceToHitTwice=>"Hit Twice",PassiveBranch.CriticalChance=>"Critical Chance",PassiveBranch.CriticalMultiplier=>"Critical Multiplier",PassiveBranch.PrecisionChance=>"Projectile Precision",PassiveBranch.PrecisionDamage=>"Precision Damage",_=>b.ToString()};
    ...
    static PassiveEffect[] TravelEffects(int s,int i)
    {
        ...
        switch(i)
        {
            ...
            case 6:effects.Add(new PassiveEffect(StatTypes.ChanceToHitTwice,1));effects.Add(new PassiveEffect(StatTypes.AuraEffect,2));break;
        }
    ...
    static PassiveEffect[] ClusterEffects(PassiveBranch branch,float amount,int sector,int cluster,bool weapon,bool notable)
    {
        ...
        if(branch==PassiveBranch.ShockChance||branch==PassiveBranch.Lightning)effects.Add(new PassiveEffect(StatTypes.ShockEffect,notable?12:5));
        if(branch==PassiveBranch.ChillChance||branch==PassiveBranch.Cold)effects.Add(new PassiveEffect(StatTypes.ChillEffect,notable?12:5));
        if(sector==4)effects.Add(new PassiveEffect(StatTypes.AuraEffect,weapon?11:notable?18:7));
