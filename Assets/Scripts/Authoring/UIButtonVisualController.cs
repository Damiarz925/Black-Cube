...
public sealed class UIButtonVisualController : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, ISelectHandler, IDeselectHandler
{
    ...
    void Awake() { if (button == null) button = GetComponent<Button>(); if (targetImage == null && button != null) targetImage = button.targetGraphic as Image; authoredScale = transform.localScale; Refresh(); }
    ...
    public void Refresh()
    {
        CurrentState = button != null && !button.IsInteractable() ? UIButtonVisualState.Disabled : selected ? UIButtonVisualState.Selected : down ? UIButtonVisualState.Pressed : over ? UIButtonVisualState.Hovered : UIButtonVisualState.Normal;
