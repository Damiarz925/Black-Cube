...
{
    GameObject panel;
    TMP_Text levelLabel, rarityLabel, modMismatchLabel;
    Button filterButton, levelToggle, rarityToggle, modMismatchToggle;
    void Start()
    {
        ...
        filterButton = Button(transform,"AUTO-DISMANTLE",new Vector2(.59f,.86f),new Vector2(.755f,.905f),TogglePanel);
        ...
        levelToggle = Button(panel.transform,"",new Vector2(.04f,.56f),new Vector2(.96f,.70f),()=>{Inventory.Instance.FilterLevelEnabled=!Inventory.Instance.FilterLevelEnabled;Refresh();});
        levelLabel = levelToggle.GetComponentInChildren<TMP_Text>();
        Button(panel.transform,"−",new Vector2(.05f,.43f),new Vector2(.27f,.55f),()=>{Inventory.Instance.FilterLevel=Mathf.Max(1,Inventory.Instance.FilterLevel-1);Refresh();});
        Button(panel.transform,"+",new Vector2(.73f,.43f),new Vector2(.95f,.55f),()=>{Inventory.Instance.FilterLevel=Mathf.Min(int.MaxValue-1,Inventory.Instance.FilterLevel)+1;Refresh();});
        rarityToggle = Button(panel.transform,"",new Vector2(.04f,.28f),new Vector2(.96f,.42f),()=>{Inventory.Instance.FilterRarityEnabled=!Inventory.Instance.FilterRarityEnabled;Refresh();});
        rarityLabel = rarityToggle.GetComponentInChildren<TMP_Text>();
        Button(panel.transform,"<",new Vector2(.05f,.15f),new Vector2(.27f,.27f),()=>{Inventory.Instance.FilterRarity=(LootManager.GearRarity)Mathf.Max(0,(int)Inventory.Instance.FilterRarity-1);Refresh();});
        Button(panel.transform,">",new Vector2(.73f,.15f),new Vector2(.95f,.27f),()=>{Inventory.Instance.FilterRarity=(LootManager.GearRarity)Mathf.Min(3,(int)Inventory.Instance.FilterRarity+1);Refresh();});
        modMismatchToggle = Button(panel.transform,"",new Vector2(.04f,.01f),new Vector2(.96f,.14f),()=>{Inventory.Instance.FilterModMismatchEnabled=!Inventory.Instance.FilterModMismatchEnabled;Refresh();});
    ...
    {var go=Box(parent,label,new Color(.14f,.19f,.22f));Place((RectTransform)go.transform,min,max);var b=go.AddComponent<Button>();b.targetGraphic=go.GetComponent<Image>();b.onClick.AddListener(action);Text(go.transform,label,0,1,13);return b;}
}
