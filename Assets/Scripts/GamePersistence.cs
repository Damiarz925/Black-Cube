...
[Serializable] public sealed class GameStatePayload
{
    ...
    public List<EndgameResourceStack> endgameResources=new(); public List<string> challengeFirstClears=new();
}
...
{
    public const string SaveKey="BlackCube.Save.V1"; public const int SchemaVersion=12;public const int CharacterSlotCount=6;
    ...
    public static bool TryCapture(out SaveEnvelope envelope,out string error)
    {
        ...
        if(EndgameResourceLedger.Instance!=null){p.endgameResources=EndgameResourceLedger.Instance.CaptureResources();p.challengeFirstClears=EndgameResourceLedger.Instance.CaptureFirstClears();}
    ...
    public static bool ValidateEnvelope(SaveEnvelope e,out string error)
    {
        ...
        int allocated=0;var seenRanks=new HashSet<int>();if(p.passiveRanks==null)return Fail("Passive allocations are missing.",out error);foreach(var r in p.passiveRanks){int id=!string.IsNullOrEmpty(r?.stableNodeId)?PassiveTreeDefinition.NodeId(r.stableNodeId):r?.nodeId??-1;if(r==null||id<0||id>=PassiveTreeDefinition.NodeCount||r.rank!=1||!seenRanks.Add(id))return Fail("Passive allocation is invalid or duplicated.",out error);allocated++;}
        if(allocated+p.availablePassivePoints!=p.playerLevel||!ValidPassiveTopology(seenRanks,p.baseClassId,p.selectedSubclassId))return Fail("Passive point accounting or topology is invalid.",out error);if(p.playerLevel<100&&p.experience>=RequirementAt(p.playerLevel)||p.playerLevel==100&&p.experience!=0)return Fail("XP does not match player level.",out error);
        ...
        return ValidateGear(p,out error)&&ValidateCurrencies(p.currencies,out error)&&ValidateEndgameResources(p,out error)&&ValidateRelics(p,out error);
    }
    static bool ValidateEndgameResources(GameStatePayload p,out string error)
    {error=null;if(p.endgameResources==null||p.challengeFirstClears==null)return Fail("Endgame resource collections are missing.",out error);var seen=new HashSet<string>();foreach(var x in p.endgameResources)if(x==null||!EndgameResourceIds.IsPersistentResource(x.resourceId)||x.amount<=0||!seen.Add(x.resourceId))return Fail("Endgame resource stack is invalid.",out error);seen.Clear();foreach(string id in p.challengeFirstClears)if(!EndgameResourceIds.IsFirstClear(id)||!seen.Add(id))return Fail("Challenge first-clear identity is invalid.",out error);return true;}
    ...
    {if(e.payload!=null)e.payload.subclassProjectileMode=SubclassProjectileMode.Volley;e.schemaVersion=10;}
    static void MigrateSchema10(SaveEnvelope e)
    {if(e.payload!=null){e.payload.endgameResources=new List<EndgameResourceStack>();e.payload.challengeFirstClears=new List<string>();}e.schemaVersion=11;}
    static void MigrateSchema11(SaveEnvelope e)
    {if(e.payload!=null){e.payload.passiveRanks=new List<PassiveRankData>();e.payload.availablePassivePoints=Mathf.Clamp(e.payload.playerLevel,1,100);}e.schemaVersion=12;}
    ...
    static bool ValidPassiveTopology(HashSet<int> allocated,string classId,string subclassId)
    {var values=new int[PassiveTreeDefinition.NodeCount];foreach(int id in allocated)values[id]=1;return PlayerProgression.ValidateAllocationState(values,classId,subclassId);}
