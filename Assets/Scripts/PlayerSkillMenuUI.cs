...
{
    [SerializeField] SkillSelectionView authoredView;
    ...
    void Start()
    {
        ...
        controller=hud.player.GetComponent<PlayerSkillController>()??hud.player.gameObject.AddComponent<PlayerSkillController>();
        if(authoredView==null)authoredView=GetComponent<SkillSelectionView>();if(authoredView==null){Debug.LogError("PlayerSkillMenuUI requires an authored SkillSelectionView.",this);return;}BindAuthored();
    ...
    void BindAuthored(){panel=authoredView.informationPanel;for(int i=0;i<2;i++){buttons[i]=i<authoredView.skillButtons.Count?authoredView.skillButtons[i]:null;labels[i]=i<authoredView.skillLabels.Count?authoredView.skillLabels[i]:null;int index=i;if(buttons[i]!=null){buttons[i].onClick.RemoveAllListeners();buttons[i].onClick.AddListener(()=>{controller.TryQueueWeaponSkill(index);Refresh();});}}rageFinisherButton=authoredView.rageFinisherButton;rageFinisherLabel=authoredView.rageFinisherLabel;if(rageFinisherButton!=null){rageFinisherButton.onClick.RemoveAllListeners();rageFinisherButton.onClick.AddListener(()=>{rage?.TryArmFinisher();Refresh();});}if(authoredView.closeButton!=null){authoredView.closeButton.onClick.RemoveAllListeners();authoredView.closeButton.onClick.AddListener(Close);}panel?.SetActive(false);IsOpen=false;}

#if UNITY_EDITOR
    public void BuildAuthoring()
    {
        hud=GetComponent<PaperBattleHUD>();if(hud==null)return;
        ...
        authoredView=GetComponent<SkillSelectionView>()??gameObject.AddComponent<SkillSelectionView>();authoredView.informationPanel=panel;authoredView.skillButtons.Clear();authoredView.skillLabels.Clear();authoredView.skillButtons.AddRange(buttons);authoredView.skillLabels.AddRange(labels);authoredView.rageFinisherButton=rageFinisherButton;authoredView.rageFinisherLabel=rageFinisherLabel;authoredView.closeButton=close;authoredView.title=title;authoredView.body=body;
    }
#endif
    ...
    void Refresh()
    {
        ...
        for(int i=0;i<2;i++)
        {
            ...
            if(skill.castMode==PlayerSkillCastMode.ImmediateCooldown){float remaining=controller.CooldownRemaining(i);bool ready=controller.ImmediateSkillReady(i);labels[i].text=ready?$"SKILL {i+1}  /  {skill.displayName.ToUpperInvariant()}  /  READY  /  {cost:0} MANA":$"SKILL {i+1}  /  {skill.displayName.ToUpperInvariant()}  /  {remaining:0.0}s";buttons[i].interactable=ready&&controller.Mana.CanSpend(cost)&&BattleManager.Instance!=null&&BattleManager.Instance.CanCastPlayerSkill;CorruptionUIButtonSkin.Ensure(buttons[i])?.SetSelected(ready);continue;}
    ...
    void OnDestroy(){IsOpen=false;if(controller!=null){controller.QueueChanged-=Refresh;controller.WeaponSkillsChanged-=Refresh;controller.CooldownsChanged-=Refresh;controller.Mana.ManaChanged-=Refresh;}if(rage!=null)rage.Changed-=Refresh;}
