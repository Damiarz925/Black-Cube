...
Weapon districts use the centralized `1.60` narrowness multiplier. Sword emphasizes tempo/critical/sustain, Axe physical Rage/retention/Bleed/recovery, Bow Precision/projectile speed/additional projectiles, Dagger speed/Poison/Void/on-hit, Staff Cast Speed/mana/elemental sustain, and Sceptre hybrid recovery/elemental defense. Weapon-restricted effects are projected only while their weapon type is equipped.
...
`PassiveTreeDefinition` is the authority for stable IDs, explicit Cartesian positions, effects, regions, edges, weapon restrictions, and extension metadata. Every node carries an empty transformation slot and stable section identity for future connected subclass transformations; no production subclass transformation is defined. Structured generation is deterministic, while the node DTO remains individually overrideable.
