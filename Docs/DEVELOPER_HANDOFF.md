...
## Step 12.5 integration note

The fresh Unity EditMode suite passes 212/212, the seeded itemization audit passes 1,312 cases, and four real-scene synchronous checks pass. Focused AttackStats, InventoryGrid, TooltipCrit and PlayerProgression fixtures, schema-3 menu/load, six-entry lifecycle/Pause and missing-reference scan pass; strict Windows x64 build succeeds with zero errors (522 existing warnings) and its standalone player remains responsive for 48 seconds. Weapon damage is now a min–max range (starter 64–96, average 80) rolled separately per real hit; noncritical previews use its average. Critical hits use a 1.5× base multiplier plus classified CritMult and show resolved life loss. Poison/Bleed/Ignite keep independent stacks; Poison schedules in global combat turns while Bleed/Ignite schedule in afflicted-actor turns. Loot/crafting/enemy items share variable tiers and Prefix/Suffix legality; direct ordinary PoE analogues, authored Black-Cube slot fallbacks and deliberate deviations are documented in [POEDB_AFFIX_BASELINE.md](POEDB_AFFIX_BASELINE.md). `GamePersistence.TrySave` now writes schema 3, stores paired damage endpoints, migrates schema-2 scalar rolls as X–X, and marks historical items to avoid invalidating old gear. Relic cards/Ancient cursors, the Void ordinary passive branch, exact-tier item tooltips and Inventory+Stats/Enemy overlay coexistence are functional. Step 6/10/11/12 counts/builds below are historical after the fresh Step 12.5 post-integration gates.
...
The persistence contract is recorded in [SAVE_STATE_CONTRACT.md](SAVE_STATE_CONTRACT.md). `GamePersistence.TrySave` writes schema-3 UTF-8 JSON under `Application.persistentDataPath` through a validated temporary file, atomic primary replacement, and one backup. Load validates before mutation, falls back primary→backup, migrates schema 2 and historical `BlackCube.Save.V1`, and restores a deterministic clean encounter start with recorded player life/mana. Pause Menu Save & Main Menu / Save & Quit remain gated on that one canonical entry point.
...
2. [PlayerController.Start](../Assets/Scripts/PlayerController.cs) equips a starter weapon if needed: base damage 64–96 (average 80), speed 1.2 attacks/second, base crit 5%. [EquipmentManager](../Assets/Scripts/EquipmentManager.cs) applies global modifiers and informs the player which weapon is equipped.
...
[CraftingCurrencySystem](../Assets/Scripts/CraftingCurrencySystem.cs) owns stackable currencies and armed-target interaction. Normal has one locked original explicit; Magic has one Prefix/one Suffix; Rare starts at three and caps at four with at most three per side; Legendary starts at five and caps at six with at most three per side. Upgrade/add/reroll/remove protect the locked original and count its side; guaranteed weapon bases are intrinsic outside those caps. All new rolls delegate to ModManager/AffixPolicy, sharing item-level, slot, group, side and family legality with drops/enemy gear. Right-click currency to arm/cancel; valid next left-click consumes one, invalid or exhausted-candidate operations consume nothing. Manual Gear/Relics tab switches cancel armed intent; automatic Ancient target switching preserves it.

[RelicSystem](../Assets/Scripts/RelicSystem.cs) owns level-50 rebirth, permanent relics, four active relic slots and relic-only Ancient crafting. Rebirth requires an explicit confirmation, resets run progression/equipment/ordinary inventory/currency, locks older relics, creates one current-cycle relic, replaces Ancient currency, and immediately checkpoints. Relic identities and active slots persist by stable ID. This permanence is within a saved game: confirmed New Game clears all relic/rebirth state. Main Menu Load accepts a valid schema-3 primary, schema-2/V1 migration or backup recovery; final button art and animated states remain future presentation work.
