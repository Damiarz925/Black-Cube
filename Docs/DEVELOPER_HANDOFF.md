...
The complete Step 6 persistence design is recorded in [SAVE_STATE_CONTRACT.md](SAVE_STATE_CONTRACT.md). Step 5 adds a runtime-built Pause Menu to `PaperBattleHUD`; Pause/Resume continue to use the existing `Time.timeScale` authority. Save & Main Menu and Save & Quit call the current canonical V1 save and refuse to leave on failure. The contract recommends one atomic current checkpoint plus backup and clean encounter-boundary restoration, but no expanded schema or storage migration is implemented yet.
...
[RelicSystem](../Assets/Scripts/RelicSystem.cs) owns level-50 rebirth, permanent relics, four active relic slots and relic-only Ancient crafting. Rebirth requires an explicit confirmation, resets run progression/equipment/ordinary inventory/currency, locks older relics, creates one current-cycle relic and replaces leftover Ancient currency with one fresh unit of each Ancient operation. Relic modifiers are limited to MORE Damage, Increased XP Gain and MORE Attack Speed with centralized conservative ranges. Only equipped relics affect formulas. [GamePersistence](../Assets/Scripts/GamePersistence.cs) saves crafted inventory/equipment/currencies, relic history and active slots as a versioned JSON PlayerPrefs snapshot. MainMenuUI creates a one-shot load request; GameManager consumes it once during normal gameplay-scene initialization and restores only this existing schema. The authored Main Menu includes a basic Load Game button, disabled when the current save key is absent; final button art and animated states remain future presentation work.
